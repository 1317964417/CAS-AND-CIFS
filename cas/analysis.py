...
test_data_loader=newData(BATCHSIZE=BATCHSIZE,name="airplane")
model_path = './modelsaved/resnet18_050_20220322.pth'
...
#     我需要的是magnitudes和counters
    for datas,labels in test_data_loader:
        ...
        outputs1 = net_std(datas, _eval=True)
        print(outputs1)
        outputs2 = net_adv(advdatas, _eval=True)
        print(outputs2)
        ...
        #################################################################
        #################################################################
        idx = np.where(labels.cpu().numpy() == np.array([0]*datas.shape[0]))[0]
        # print(idx)
        #################################################################
        #################################################################
        ...
        if(len(idx)>0):
            ...
            for k in range(C):
                if feat_mean_magnitude[k] != 0:
                    feat_mean_magnitude[k] = feat_mean_magnitude[k] / (COUNTER_ACTIVATE[k].float()*max_value[0][k].float())
    ...
    np.save('Frequency/cifar10_std_layer4_class0_1e-2_1.npy', res)
    ...
    np.save('Magnitude/cifar10_std_layer4_class0_1e-2_1.npy', res)
