...
def topk_defense_success_rate(predict_for_test, predict_for_atk, loader, attack_class, attack_kwargs,
    ...
    accuracy, defense_success_rate, dist, num = attack_mini_batches(predict_for_test, adversary, loader, device=device,
                                                                    norm=None, num_batch=num_batch, topk=topk)

    # message returned
    message = '***** Test set acc: {:.2f}%, std: {:.2f}%.'.format(
        accuracy * 100.,
...
def attack_mini_batches(myPredict,
    ...
    for data, label in tqdm(loader):
        ...
        adv_logits = myPredict(adv)
        advpred = predict_from_logits_topk(adv_logits, topk=topk)
        nat_logits = myPredict(data)
        pred = predict_from_logits_topk(nat_logits, topk=topk)
...
def benchmark_defense_success_rate(predict_for_test, predict_for_atk, loader, attack_class, attack_kwargs,
    ...
    adversary = attack_class(predict_for_atk, **attack_kwargs)
    label, pred, advpred, dist = multiple_mini_batch_attack(predict_for_test, adversary, loader, device=device,
                                                            norm=None, num_batch=num_batch)
