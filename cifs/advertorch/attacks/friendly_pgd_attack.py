...
def earlystop(model, data, target, step_size, epsilon, perturb_steps,tau,randominit_type,loss_fn,rand_init=True,omega=0):
    ...
    :param tau: the step controlling how early we should stop interations when wrong std data is found
    :param randominit_type: To decide the type of random inirialization (random start for searching std data)
    :param rand_init: To decide whether to initialize adversarial sample with random noise (random start for searching std data)
    :param omega: random sample parameter for std data generation (this is for escaping the local minimum.)
    ...
    while K>0:
        ...
        if len(output_index) != 0:
            if len(output_target) == 0:
                # incorrect std data should not keep iterated
